...
дублей (и старое, и новое API) начинает работать для таких записей.

Приложение мультитенантное (django-tenants), поэтому команду нужно запускать
в контексте конкретной схемы:

    python manage.py tenant_command backfill_guests --schema=demo
    python manage.py tenant_command backfill_guests --schema=demo --dry-run

Для всех тенантов сразу:

    python manage.py all_tenants_command backfill_guests
"""
